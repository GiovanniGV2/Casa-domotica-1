...
#define SERVO_PIN 5
#define LED_PIN 15
#define LED_ROJO_PIN 23 // Pin para el LED rojo
...
int readGasMapped() {
    ...
    Serial.println(raw);
    // Nota: El rango de 0 a 4095 es el rango ADC predeterminado del ESP32.
    // El mapeo a 0-5000 es arbitrario y depende del sensor y calibración deseada.
...
void setup() {
    ...
    // El GAS_PIN (GPIO 34) no necesita pinMode() ya que se usa para lectura analógica
...
void loop() {
    ...
    if (now - lastSensorRead >= SENSOR_INTERVAL) {
        ...
        // 🚨 Lectura de Gas (llama a readGasMapped() que imprime el valor crudo)
        d.gas = readGasMapped();
